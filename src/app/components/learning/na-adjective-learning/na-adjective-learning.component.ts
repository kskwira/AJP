...
@Component({
  selector: 'app-na-adjective-learning',
  templateUrl: './na-adjective-learning.component.html',
  styleUrls: ['./na-adjective-learning.component.css']
})
export class NaAdjectiveLearningComponent implements OnInit {

  userData: any; // Save logged in user data
  currentUser?: UserModel;

  naAdjectiveQuizArray: Vocabulary[] = [];
  naAdjectiveLearnArray: Vocabulary[] = [];
  nineAnswers: Vocabulary[] = [];
  allNaAdjectivesList: Vocabulary[] = [];
  ...
  getAllNaAdjectives(): void {
    this.kanaService.getAllNaAdjectives().snapshotChanges().pipe(
      ...
      this.allNaAdjectivesList = data;
    });
  }

  GetNaAdjectivesByLevel(level: number): void {
    this.kanaService.getNaAdjectivesByLevel(level).snapshotChanges().pipe(
      map(changes =>
        changes.map(c =>
          ({id: c.payload.doc.id, ...c.payload.doc.data()})
        )
  ...
  prepareAnswers(id: Vocabulary[]) {
    ...
    naAdjectiveIdSet.add(this.randomizedIdArray[this.randomizedIdArrayIndex])
  ...
  learnSession(id: number): void {
    if (this.sortedIdArrayIndex == 0) {
      this.sortedIdArray = [...this.naAdjectivesByLevelUidSet]
    ...
    if (this.sortedIdArrayIndex <= this.sortedIdArray.length) {
      this.kanaService.getSingleNaAdjectiveById(id).snapshotChanges().pipe(
        ...
        this.naAdjectiveLearnArray = data;
      });
    }
    else {
      this.quizStart = true;
      ...
      this.result = '';
      this.icon = '';
      this.naAdjectiveLearnArray.splice(0, this.naAdjectiveLearnArray.length);
      this.kanaService.getSingleNaAdjectiveById(id).snapshotChanges().pipe(
        map(changes =>
          changes.map(c =>
            ({id: c.payload.doc.id, ...c.payload.doc.data()})
        ...
        this.naAdjectiveQuizArray = data;
      });
      this.prepareAnswers(this.allNaAdjectivesList)
    }
  }
