...
@Component({
  selector: 'app-noun-learning',
  templateUrl: './noun-learning.component.html',
  styleUrls: ['./noun-learning.component.css']
})
export class NounLearningComponent implements OnInit {

  userData: any; // Save logged in user data
  currentUser?: UserModel;

  nounQuizArray: Vocabulary[] = [];
  nounLearnArray: Vocabulary[] = [];
  nineAnswers: Vocabulary[] = [];
  allNounsList: Vocabulary[] = [];
  ...
  getAllNouns(): void {
    this.kanaService.getAllNouns().snapshotChanges().pipe(
      ...
      this.allNounsList = data;
    });
  }

  GetNounsByLevel(level: number): void {
    this.kanaService.getNounsByLevel(level).snapshotChanges().pipe(
      map(changes =>
        changes.map(c =>
          ({id: c.payload.doc.id, ...c.payload.doc.data()})
        )
  ...
  prepareAnswers(id: Vocabulary[]) {
    ...
    nounIdSet.add(this.randomizedIdArray[this.randomizedIdArrayIndex])
  ...
  learnSession(id: number): void {
    if (this.sortedIdArrayIndex == 0) {
      this.sortedIdArray = [...this.nounsByLevelUidSet]
    ...
    if (this.sortedIdArrayIndex <= this.sortedIdArray.length) {
      ...
      this.result = '';
      this.icon = '';
      this.nounLearnArray.splice(0, this.nounLearnArray.length);
      this.kanaService.getSingleNounById(id).snapshotChanges().pipe(
        map(changes =>
          changes.map(c =>
            ({id: c.payload.doc.id, ...c.payload.doc.data()})
        ...
        this.nounQuizArray = data;
      });
      this.prepareAnswers(this.allNounsList)
    }
  }
